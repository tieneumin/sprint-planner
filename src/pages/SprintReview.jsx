...
const SprintReview = () => {
  ...
  const { activeSprint, completeSprint, getSprintProgress } = useSprintManager();
  ...
  const today = dayjs();
  const isSprintCompleted = today.isAfter(endDate) || today.isSame(endDate, 'day');
  ...
  const handleCompleteSprint = () => {
    ...
    const completedData = {
      ...
      reflections,
      rating: sprintRating,
      finalCompletionRate: (Object.values(goalStatuses).filter(s => s === 'completed').length / activeSprint.goals.length) * 100
    };
  ...
  const getTotalEstimatedHours = () => {
    return activeSprint.goals.reduce((sum, goal) => sum + goal.estimatedHours, 0);
  };

  const getTotalActualHours = () => {
    return activeSprint.goals.reduce((sum, goal) => sum + (goal.actualHours || 0), 0);
  };
      ...
      <Paper sx={{ p: 4 }}>
        ...
        {!isSprintCompleted && (
          <Alert severity="info" sx={{ mb: 3 }}>
            Your sprint isn't scheduled to end until {endDate.format('MMMM D, YYYY')}, 
            but you can complete it early if needed.
          </Alert>
        )}
              ...
              <Card key={goal.id} variant="outlined" sx={{ mb: 2 }}>
                <CardContent>
                  <Box display="flex" alignItems="flex-start" gap={2}>
                    <Box>
                      {getStatusIcon(goalStatuses[goal.id])}
                    </Box>
                    ...
                    <Box flex={1}>
                      ...
                      <Box display="flex" gap={1} mb={2}>
                        ...
                        />
                        <Chip 
                          label={`${(goal.actualHours || 0).toFixed(1)}h actual`} 
                          size="small" 
                          variant="outlined"
                        />
                      </Box>
          ...
          <Grid item xs={12}>
            <Box display="flex" justifyContent="space-between" alignItems="center" sx={{ mt: 3 }}>
              ...
              <Button
                ...
                disabled={
                  Object.keys(goalStatuses).length !== activeSprint.goals.length ||
                  sprintRating === 0
                }
                color="success"
              >
