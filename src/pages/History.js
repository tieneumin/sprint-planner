...
const History = () => {
  ...
  const getOverallStats = () => {
    ...
    const totalGoals = completedSprints.reduce(
      (sum, sprint) => sum + sprint.goals.length,
      0
    );
    const completedGoals = completedSprints.reduce((sum, sprint) => {
      return (
        sum +
        sprint.goals.filter(
          (goal) => goal.finalStatus === "completed" || goal.completed
        ).length
      );
    }, 0);
    ...
    const totalEstimatedHours = completedSprints.reduce((sum, sprint) => {
      return (
        sum +
        sprint.goals.reduce((goalSum, goal) => goalSum + goal.estimatedHours, 0)
      );
    }, 0);
    ...
    const totalActualHours = completedSprints.reduce((sum, sprint) => {
      return (
        sum +
        sprint.goals.reduce(
          (goalSum, goal) => goalSum + (goal.actualHours || 0),
          0
        )
      );
    }, 0);
    ...
    const averageRating =
      completedSprints.reduce((sum, sprint) => sum + (sprint.rating || 0), 0) /
      totalSprints;
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4 }}>
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        ...
        <Grid container spacing={2}>
          <Grid item xs={6} sm={2}>
            ...
            <Typography variant="h6">{stats.totalSprints}</Typography>
          </Grid>
          ...
          <Grid item xs={6} sm={2}>
            ...
            <Typography variant="h6">{stats.totalEstimatedHours}h</Typography>
          </Grid>
      ...
      {completedSprints
        ...
        .map((sprint, index) => {
          const completedGoals = sprint.goals.filter(
            (goal) => goal.finalStatus === "completed" || goal.completed
          ).length;
          const completionRate =
            sprint.goals.length > 0
              ? (completedGoals / sprint.goals.length) * 100
              : 0;
                ...
                >
                  <Box>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      ...
                      {" • "}
                      Completed{" "}
                      {dayjs(sprint.completedAt).format("MMM D, YYYY")}
                    </Typography>
                  </Box>
